    ...
    <el-dropdown @command="handleCommand">
      ...
      <el-dropdown-menu slot="dropdown" class="tags-more">
        ...
        <el-dropdown-item command="closeRightTags">
          <vab-icon :icon="['fas', 'arrow-alt-circle-right']"></vab-icon>关闭右侧
        </el-dropdown-item>
...
export default {
  ...
  methods: {
    async handleTabRemove(tabActive) {
      ...
      if (view && view.meta && view.meta.closeTips) {
        //再判断单据状态
        // debugger
        let __status = this.$store.state.gl_vouchercard.voucherStatus
        if (__status) {
          await this.$confirm(`${view.meta.title}尚未保存，还要离开吗?`, "提示", {
